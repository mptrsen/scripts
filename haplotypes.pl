...
my $seq_of = Seqload::Fasta->slurp_fasta($af);

my $seen = {};

while (my ($h, $s) = each %$seq_of) {
	my @fields = split $h;
	if ($seen->{$fields[0]}->{$fields[1]} and $seen->{$fields[0]}->{$fields[1]}->{'seq'} eq $s) {
		printf "seen %s from %s before, skipping\n", $fields[0], $fields[1];
	}
	$seen->{$fields[0]}->{$fields[1]}->{'n'}++;
}

# Documentation for the Seqload::Fasta module 
=head1 NAME
