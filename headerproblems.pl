...
my $logf = shift @ARGV;
die "Usage: $0 [options] HAMSTRAD_LOG_FILE\n" unless $logf;
...
# read the sequence headers from the sequence file
printf "%-18s %s\n", 'sequence file:', $seqf;
my @headers = slurp_fasta($seqf);
my @lengths = ();
push @lengths, length($_) foreach @headers;

# report on min and max header length
printf "%-18s %d\n", 'max header length:', max(@lengths);
printf "%-18s %d\n", 'min header length:', min(@lengths);

# ok we'll go through the log file now
$fh = IO::File->new($logf);
while (<$fh>) {
	...
	if ( /processing hit: (.*)_RF\d/ ) {
		...
		@matches = grep /$hitseq/, @headers;
		if (scalar @matches > 1) {
			print "Warning: potential error source: $hitseq matches multiple times: @matches\n";
			$faulty = 1;
		}
		else { print "OK\n" if $verbose }
		next;
	...
	# is a skipped sequence header a substring of multiple sequence headers?
	if ( /(.*) and (.*) returned/ ) {
		$nucseq = $1;
		$coseq = $2;
		@matches = grep /$nucseq/, @headers;
		if (scalar @matches > 1) {
			print "skipped $nucseq and $coseq: $nucseq matches multiple times: @matches", "\n";
		}
		next;
	}
...
if ($faulty) {
	print "found problematic sequences\n";
}
else {
	print "everything OK\n";
}
