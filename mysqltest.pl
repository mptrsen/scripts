...
my @bin = ( );
my $sum = 0;
my $query_offset = "select o_hmmsearch.evalue, o_orthologs.ortholog_gene_id, o_hmmsearch.start, o_hmmsearch.end, o_blast.target, o_blast.evalue, o_taxa.name from o_logevalues left join o_hmmsearch on o_logevalues.log_evalue = o_hmmsearch.log_evalue left join o_ests on o_hmmsearch.target = o_ests.digest left join o_orthologs on o_hmmsearch.query = o_orthologs.ortholog_gene_id left join o_blast on o_hmmsearch.target = o_blast.query left join o_aaseqs on o_blast.target = o_aaseqs.id left join o_taxa on o_aaseqs.taxid = o_taxa.id  left join o_set_details on o_orthologs.setid = o_set_details.id where o_set_details.id='1' and o_hmmsearch.taxid = '45' limit='?' offset='?'";
